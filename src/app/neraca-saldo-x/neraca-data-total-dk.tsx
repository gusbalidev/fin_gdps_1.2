...
const NeracaDataTotalDK = ({ start, end }: { start: string; end: string }) => {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`/api/neraca-saldo-total-dk?startDate=${start}&endDate=${end}`, {
        cache: "no-store",
      }).then((response) => {
    ...
    <>
      <div className="flex flex-row">
        ...
        <div className="w-3/4 pr-2">
          <Divider />
          <div className="pr-2">
            <p className="text-start text-blue-600 dark:text-orange-600 font-bold">
              TOTAL MUTASI
            </p>
          </div>
        </div>
        <div className="flex flex-row w-3/5 pr-2">
        ...
        <div className="w-1/4">
          <div className="text-blue-600 dark:text-orange-600 font-bold">
            <Divider />
            {/* <SubTotalDK value={newTotalSaldo} /> */}
            <p></p>
          </div>
        </div>
      </div>
