...
export function EditDialog2({ children, account, onSuccess }: EditDialogProps) {
  ...
  // const idrBalance = 
  //   balance1 < 0
  //     ? `-Rp. ${Math.abs(balance1).toLocaleString("id-ID")}`
  //     : `Rp. ${balance1.toLocaleString("id-ID")}`;
        ...
        <DialogContent className="sm:max-w-[425px] bg-white dark:bg-gray-800">
          ...
          <form action={handleFormAction} className="space-y-4">
            ...
            <div className="grid gap-4 py-4">
              ...
              {/* <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="accountGroup" className="text-left">
                  Grup Akun
                </Label>
                <Select
                  name="accountGroupId"
                  value={String(account.accountGroupId)}
                  onValueChange={(value) => {
                    console.log("Selected value:", value);
                  }}
                >
                  <SelectTrigger id="accountGroup" className="col-span-3">
                    <SelectValue placeholder="Pilih grup akun">
                      {
                        accountGroups?.find(
                          (group) =>
                            String(group.id) === String(account.accountGroupId)
                        )?.name
                      }
                    </SelectValue>
                  </SelectTrigger>
