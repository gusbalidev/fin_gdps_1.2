...
function WidgetInfoTotalNew() {
    ...
    //const totAT = totalAT;
    //const totalAktiva = totalA + totalAL + totalATL;
    
    //const totalABersih = totalAT - totalAP;

    //setTotalAT(totalAT);
    //setTotalAB(totalABersih)
    //setTotalAB2(totalAB2)
    ...
    return (
        ...
        <div className="p-4 rounded-lg bg-blue-50 dark:bg-slate-800">
            ...
            {isBalanceSheetEqual(totalAktiva, totalPasiva) ? 
                <p className="text-green-500 pb-2">{global.pageInfo.infoNeracaBalance}</p> 
                : 
                <p className="text-red-500 pb-2">{global.pageInfo.infoNeracaUnbalance}</p>}
            ...
            <TulisTotalRpBig value={toidr(totalPasiva)} title="KEWAJIBAN dan ASET BERSIH" />

            {isBalanceSheetEqual(totalAktiva, totalPasiva) ?
                <TulisTotalRpGreen value={selisihAkhir} title="Selisih" />
                :
                <TulisTotalRpRed value={selisihAkhir} title="Selisih" />
            }
        </div>
    )
...
export default WidgetInfoTotalNew