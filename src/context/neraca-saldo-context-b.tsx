...
const NeracaSaldoContext = createContext
    <{
        ...
        periodType2: 'M' | 'Y'; setPeriodType2: (type: 'M' | 'Y') => void;
    ...
    ({
        ...
        periodType2: 'M', // 'M' for monthly, 'Y' for yearly
...
export function NeracaSaldoProviderB({ children }: {
    ...
    const [periodType2, setPeriodType2] = useState<'M' | 'Y'>('M');
...
export default function useNeracaSaldoContextB() {
    const context = useContext(NeracaSaldoContext);
    if (!context) {
        throw new Error(
            "useNeracaSaldoContextB must be used within a NeracaSaldoProvider"
        );
    }
    return context;
}
