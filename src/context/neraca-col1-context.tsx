...
const NeracaCol1Context = createContext
    <{
...
export function NeracaCol1Provider({ children }: {
    ...
    return (
        <NeracaCol1Context.Provider value={{
            saldo1, setSaldo1,
            ...
            {children}
        </NeracaCol1Context.Provider>
    )

}

export default function useNeracaCol1Context() {
    const context = useContext(NeracaCol1Context);
    if (!context) {
        throw new Error(
            "useNeracaCol1Context must be used within a NeracaCol1Provider"
        );
    }
    return context;
}
